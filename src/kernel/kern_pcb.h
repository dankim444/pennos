...
typedef struct pcb_st {
  ...
  int fd_table[FILE_DESCRIPTOR_TABLE_SIZE];  // file descriptor table (-1 if not
                                             // in use)
} pcb_t;
...
/**
 * @brief Creates a new PCB and initializes its fields. Notably, the thread
 * handle and cmd are left out. It's up to the user to assign them post-call.
 *
 ...
 * parent. Also inserts the created child into the correct scheduler queue based
 * on its priority.
