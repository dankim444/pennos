...
//   curr_block = follow_fat_chain(metadata.first_block,
//   sys_fd_table[fd].offset,
//                                 &block_offset);

//   int block_size = fat_get_block_size();
//   int bytes_read = 0;

//   while (bytes_read < bytes_to_read && curr_block != FAT_EOF &&
//          curr_block != FAT_FREE) {
...
//     memcpy(buf + bytes_read, (char*)block_data + block_offset,
//            block_bytes_to_read);
...
//       uint16_t last_block = follow_fat_chain(
//           metadata.first_block, metadata.size - 1, &last_block_offset);
...
//     curr_block =
//         follow_fat_chain(metadata.first_block, position, &block_offset);
//   }
...
//     memcpy((char*)block_data + block_offset, str + bytes_written,
//            block_bytes_to_write);
...
//                (metadata.permissions & 0x01) ? 'r' : '-', metadata.size,
//                (unsigned int)metadata.timestamp, metadata.filename);
//         count++;
