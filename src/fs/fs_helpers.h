 ...
 * Purpose: Defines the global variables for the mounted filesystem and
 * additional helpers to implement the routines.
 */
...
extern uint16_t*
    fat;  // pointer to the FAT region in memory, for efficient access
 ...
 * @return 1 if the file has executable permissions, 0 if it doesn't, -1 if an
 * error occurred.
 */
