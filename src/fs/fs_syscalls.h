 ...
 * Purpose: Defines the system-level file system calls.
 */
 ...
 *
 * This function provides a user-level interface to the kernel's file open
 * operation. It opens the specified file with the given access mode and returns
 * a file descriptor that can be used in subsequent operations on the file.
 *
 * @param fname The name of the file to open.
 * @param mode  A combination of F_READ, F_WRITE, and F_APPEND.
 *
 * @return On success, returns a non-negative integer representing the file
 * descriptor. On error, returns -1 and sets P_ERRNO appropriately:
 ...
 *
 * This function reads up to n bytes from the file associated with the file
 * descriptor fd into the buffer starting at buf. The file offset is advanced by
 ...
 *
 * @return On success, returns the number of bytes read (0 indicates end of
 * file). On error, returns -1 and sets P_ERRNO appropriately:
 *         - P_EBADF: fd is not a valid file descriptor or is not open for
 * reading.
 ...
 *
 * This function writes up to n bytes from the buffer starting at str to the
 * file associated with the file descriptor fd. The file offset is advanced by
 ...
 *
 * This function repositions the offset of the file descriptor fd to the
 * argument offset according to the directive whence.
 *
 * @param fd      The file descriptor of an open file.
 * @param offset  The offset in bytes.
 * @param whence  Specifies the reference position:
 *                - SEEK_SET (0): The offset is set relative to the start of the
 * file.
 *                - SEEK_CUR (1): The offset is set relative to the current
 * position.
 *                - SEEK_END (2): The offset is set relative to the end of the
 * file.
 *
 * @return On success, returns the resulting offset from the beginning of the
 * file. On error, returns -1 and sets P_ERRNO appropriately:
 *         - P_EBADF: fd is not a valid file descriptor.
 *         - P_EINVAL: whence is not valid or the resulting offset would be
 * negative.
 */
 ...
 * If filename refers to a specific file, it displays detailed information about
 * that file.
 *
 * @param filename The name of the file to get information about, or NULL to
 * list all files.
 *
 * @return On success, returns 0.
 *         On error, returns -1 and sets P_ERRNO appropriately:
